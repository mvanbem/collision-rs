...
fn benchmark_insert(b: &mut Bencher) {
    ...
    let mut i = 0;
    b.iter(|| {
        let offset = rng.gen_range(0.0..100.);
...
fn benchmark_query(b: &mut Bencher) {
    ...
    for i in 0..100000 {
        ...
        let offset_x = neg_x * rng.gen_range(9000.0..10000.);
        let offset_y = neg_y * rng.gen_range(9000.0..10000.);
        tree.insert(Value::new(
            i,
    ...
    let rays: Vec<Ray2<f32>> = (0..1000)
        .map(|_| {
            let p = Point2::new(
                rng.gen_range(-10000.0..10000.),
                rng.gen_range(-10000.0..10000.),
            );
            let d = Vector2::new(rng.gen_range(-1.0..1.), rng.gen_range(-1.0..1.)).normalize();
            Ray2::new(p, d)
        })
        .collect();
